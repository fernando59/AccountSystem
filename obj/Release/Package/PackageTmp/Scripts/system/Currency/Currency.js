...
function validate(exchange) {
    ...
    generadorAlertas('error', 'Error', 'Tiene que ingresar un tipo de cambio')
        return false
    } 
    if (exchange < 0) {
    ...
    if (listCompanyCurrency[0].exchange == exchange) {
            generadorAlertas('error', 'Error', 'No puede existir dos monedas seguidas con el mismo valor ')
        return false
    }
    return true
